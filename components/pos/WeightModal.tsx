  ...
  return (
    <div className="no-print fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
      <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
        ...
        <p className="mt-2 text-sm opacity-55">Kg fiyatı: {money(item.price ?? 0)} ₺</p>
        <input
          autoFocus
          type="number"
          min="1"
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          <button type="button" onClick={onCancel} className="rounded-xl border border-black/10 px-4 py-3">
            Vazgeç
          </button>
          <button type="button" onClick={onAdd} className="rounded-xl bg-[#6e1f12] px-4 py-3 font-bold text-white">
            Sepete Ekle
          </button>
        </div>
      </div>
